class GoalPostComment < ActiveRecord::Base
  ...
  validates_presence_of :account_id, :goal_post_id, :content
  
  validates_length_of :content, :maximum => 1000, :message => "回帖内容 超过长度限制", :allow_nil => false
  
  
  
  CKP_count = :goal_post_comment_count
  Count_Cache_Group_Field = :goal_post_id
