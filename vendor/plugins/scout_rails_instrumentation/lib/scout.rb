...
class Scout
  ...
  # Rails versions 2.3.0+ benchmark view in ms, not sec
  # 2.2.2 - http://github.com/rails/rails/blob/ff561370739eb809d1d006ea842cd2b1e43f6950/actionpack/lib/action_controller/benchmarking.rb
  # 2.3.0 - http://github.com/rails/rails/blob/beca1f2e151558ded3d5a4efebd328ab2533edc6/actionpack/lib/action_controller/benchmarking.rb
  VIEW_BENCHMARKING_IN_SECONDS = (RAILS_GEM_VERSION =~ /\A(?:1\b|2\.[0-2]\b)/)
  
  # Rails versions 2.2+ benchmark total in ms, not sec
  TOTAL_BENCHMARKING_IN_SECONDS = (RAILS_GEM_VERSION =~ /\A(?:1\b|2\.[0-1]\b)/)
  ...
  class << self
    ...
    def record_metrics(runtimes, params, response, options = {})
      ...
      # Rails versions lower than 2.3.0 benchmark in seconds - need to convert to mss
      fix_runtimes_to_ms!(runtimes)
    ...
    def log!(level, message)
      if RAILS_ENV == "production" and [:error, :fatal].include?(level)
        ...
        # Early versions of Rails just use Logger instead of the BufferedLogger
    ...
    def fix_runtimes_to_ms!(runtimes)
      runtimes[:total] = seconds_to_ms(runtimes[:total]) if TOTAL_BENCHMARKING_IN_SECONDS
      runtimes[:view] = seconds_to_ms(runtimes[:view]) if VIEW_BENCHMARKING_IN_SECONDS
      return runtimes
    end
